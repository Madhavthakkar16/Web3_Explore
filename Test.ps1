...
try {
  ...
  Write-Host "⚠ Could not set ExecutionPolicy (scope CurrentUser). Continuing..."
}
...
if ($LASTEXITCODE -eq 0) {
  ...
  if ($status -eq 'running') {
    Say "♻ Container '$NAME' already running; reusing."
  } else {
...
Say "🛠 Deploying contract for '$challenge'..."
...
if (!fs.existsSync(path)) {
  console.log('⚠ ' + path + ' not found; please paste manually.');
  process.exit(0);
}
...
"@
  ...
  Say "⚠ Could not detect contract address automatically. Copy it from logs and paste manually in the debug UI file."
}
